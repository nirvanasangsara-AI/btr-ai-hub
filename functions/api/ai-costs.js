...
export async function onRequestGet({ request, env }) {
  ...
  try {
    if (env.OPENROUTER_API_KEY) {
      ...
      if (r.ok) {
        ...
        const data = d.data || d;
        services.openrouter = {
          usage_usd: data.usage_monthly || data.usage || null,
  ...
  try {
    if (env.ANTHROPIC_API_KEY) {
      const r = await fetch(
        `https://api.anthropic.com/v1/usage?start_date=${startDate}&limit=100`,
        {
          headers: {
      ...
      if (r.ok) {
        ...
        // 평균 가격: $3/M input, $15/M output, $3.75/M cache
  ...
  // ── 3. DeepSeek — API 자동조회, fallback $9 ──────────
  try {
    if (env.DEEPSEEK_API_KEY) {
      ...
      if (r.ok) {
        ...
        services.deepseek = { balance_usd: 9, balance_cny: null, source: 'fixed', note: 'API 실패' };
      }
    } else {
